...
describe('api cache', () => {
  ...
  it('should cache based on the map response', (done) => {
    ...
    api.post('/blogs/entries', [ 'id1', 'id9', 'id23', 'id73' ]).then(() => {
      ...
      expect(api.mappedHandler.get(requestKey, [ 'id1', 'id23' ])).toEqual(new Map([
        [ 'id1', 'Blog 1' ], [ 'id23', 'Blog 23' ]
      ]));

      expect(api.mappedHandler.get(requestKey, [ 'id9', 'id73', 'id99' ])).toEqual(new Map([
        [ 'id9', 'Blog 9' ], [ 'id73', 'Blog 73' ]
      ]));
  ...
  it('should fetch missing entries', (done) => {
    ...
    api.request(request).then(resp => {
      ...
      expect(resp).toEqual(new Map([
        [ 'id1', 'Blog 1' ], [ 'id2', 'Blog 2' ], [ 'id3', 'Blog 3' ], [ 'id4', 'Blog 4' ]
      ]))
      done();
    });
  });
