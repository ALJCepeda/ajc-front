...
type FormWithActionOptions<
  IStoreState,
  IPayloadType extends IEntity,
  IResponseType
> = FormOptions<IPayloadType> & {
  storeActions: (
    form: Form<IPayloadType>
  ) => { [type: string]: Action<IStoreState, IPayloadType | any, any> };
};
...
export async function fromAction<
  IStoreState,
  IRequestType,
  ILoadType,
  IResponseType
>(
  ...
  if (!action.payload) {
    throw new Error(
      'Cannot load form without a payload assigned to action. Did you forget to call "with" on the action?'
    );
  }
