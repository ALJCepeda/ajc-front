<template>
  <main class="sform">
    <div>
      ...
      ></sinput>
      <div v-if="form.controls.length === 0">
        No controls for form to render!
      </div>
    </div>
    ...
    <div class="row-nw jc-center action-btns">
      ...
      <button class="btn btn-danger remove" v-if="entry.id" @click="remove()">
        Delete
      </button>
    </div>
  </main>
...
@Component
export default class FormComponent<IResourceType> extends AbstractFormComponent<
  IResourceType
> {
  name = "sform";
